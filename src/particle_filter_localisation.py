...
class ParticleFilter:
    def __init__(self):
        ...
        self.num_particles_ = rospy.get_param("~num_particles", 100) # Number of particles
        self.num_motion_updates_ = rospy.get_param("~num_motion_updates", 6) # Number of motion updates before a sensor update
        self.num_scan_rays_ = rospy.get_param("~num_scan_rays", 6) # (Approximate) number of scan rays to evaluate
        self.num_sensing_updates_ = rospy.get_param("~num_sensing_updates", 6) # Number of sensing updates before resampling
    ...
    def initialise_particles(self):
        ...
        # for loop iterates through every particle 
        # and initialises x coordinate, y coordinate, orientation and weight
	    # .append method adds a Particle object with the initialised values to the particles array 
    ...
    def normalise_weights(self):
        ...
        # calculate each normalised weight 
        # by dividing the current weight value by the sum of the weights
        for p in self.particles_:
            p.weight = p.weight / weight_sum

        # use particles_ array instead of num_particles_, because particles out of the map are getting deleted
        # odom_callback method deletes the particles and stores the remaining particles in particles_ array
    ...
    def estimate_pose(self):
        ...
        # introduce auxiliary variables for the cartesian coordinates of the angle
        theta_cart_x = 0
        theta_cart_y = 0

        # calculate the weighted average over all particles 
        # of the x coordinate, y coordinate and the cartesian coordinates of the angle
        for p in self.particles_:
            estimated_pose_x += p.weight * p.x
            estimated_pose_y += p.weight * p.y
            theta_cart_x += p.weight * math.cos(p.theta)
            theta_cart_y += p.weight * math.sin(p.theta)
        
        # convert the cartesian coordinates back to the angle
        estimated_pose_theta = wrap_angle(math.atan2(theta_cart_y, theta_cart_x))
	...
	# iterate through all particles p
	for p in self.particles_:

            # get distance and rotation noise
            ...
            # store x, y, theta value at step t (for overview purposes)
            x_t = p.x
            y_t = p.y
            theta_t = wrap_angle(p.theta)

            # calculate x, y, theta at step t+1 using provided formula
            x_t1 = x_t + (distance + distance_noise) * math.cos(theta_t)
            y_t1 = y_t + (distance + distance_noise) * math.sin(theta_t)
            theta_t1 = wrap_angle(theta_t + rotation + rotation_noise)

            # store x, y, theta at step t+1 in every particle (for overview purposes)
            p.x = x_t1
            p.y = y_t1
            p.theta = theta_t1
    ...
    def scan_callback(self, scan_msg):
		...
		# compare range of every scan i with expected range of particle p to get ray likelihood
		# divided the formula in normalisation (norm) and exponential (exp) part (for overview purposes)
                norm = 1 / (math.sqrt(2*math.pi*self.sensing_noise_stddev_**2))
                exp = math.exp(-(((particle_range-scan_range)**2)/(2*self.sensing_noise_stddev_**2)))
                ray_likelihood = norm * exp

                # multiply likelihood of every scan i to get the likelihood of particle p
                likelihood *= ray_likelihood

                # for every particle p multiply likelihood with the old weight to get the new weight
            p.weight *= likelihood
